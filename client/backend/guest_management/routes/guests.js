...
router.patch("/:guestId/status", async (req, res) => {
  try {
    ...
    const validStatuses = ["inquiry", "booked", "rsvp_sent", "rsvp_completed", "arrived", "cancelled"];
    if (!validStatuses.includes(status)) {
      return res.status(400).json({ error: `Invalid status. Must be one of: ${validStatuses.join(", ")}` });
    }

    // If setting to "booked", auto-generate RSVP token + QR code
    if (status === "booked") {
      ...
      return res.json({
        message: "Guest booked. QR code and RSVP link generated.",
        guestId,
        rsvpToken,
