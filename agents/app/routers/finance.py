...
async def init_installment_plan(payload: InitPlanRequest, db: AsyncSession = Depends(get_db)):
    ...
    # Map event menu tier to a hardcoded base rate per guest
    tier_rates = {
        "standard": Decimal("1500.00"),
        "premium": Decimal("2500.00"),
        "elite": Decimal("4000.00"),
    }
    base_rate_per_guest = tier_rates.get(event.menu_tier.value.lower(), Decimal("2000.00"))
...
async def confirm_event(payload: ConfirmEventRequest, db: AsyncSession = Depends(get_db)):
    ...
    # ─── Callback: Notify WhatsApp Bot to move customer to DONE ───
    bot_notified = False
    try:
        bot_url = os.getenv("BOT_WEBHOOK_URL", "http://localhost:5556")
        async with httpx.AsyncClient(timeout=10.0) as client:
            bot_response = await client.post(
                f"{bot_url}/finance/confirm-payment",
                json={"phone": event.client_phone, "payment_confirmed": True},
            )
            bot_notified = bot_response.status_code == 200
            logger.info(f"Bot callback response: {bot_response.status_code} - {bot_response.text}")
    except Exception as e:
        logger.warning(f"Bot webhook callback failed: {e}")
    ...
    return ConfirmationResponse(
        ...
        po_generated=True,
        whatsapp_sent=whatsapp_sent or bot_notified,
    )
