...
async function generateQRForGuest(guestId) {
  ...
  if (!["booked", "verified", "rsvp_sent", "rsvp_completed"].includes(guest.status)) {
    throw new Error(`Guest status is "${guest.status}", must be "booked", "verified", "rsvp_sent", or "rsvp_completed" to generate QR`);
  }
