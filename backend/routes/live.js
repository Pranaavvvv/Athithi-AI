...
router.post("/dj/:eventId/request", async (req, res) => {
  try {
    ...
    // Guest already submitted a song for this event — one per link policy
    ...
    const displayName = result.normalized_song || song_name;
    res.status(result.action === "created" ? 201 : 200).json({
      message: result.action === "created"
        ? `Song "${displayName}" added to queue!`
        : `Song "${displayName}" upvoted! (${result.request.upvotes} votes)`,
      ...result,
    });
  } catch (err) {
...
// ─── Kitchen Food Requests: Submit (Auto-Dedup) ────────────────

router.post("/kitchen/:eventId/request", async (req, res) => {
  try {
    const { eventId } = req.params;
    const { item_name, requested_by } = req.body;

    if (!item_name) {
      return res.status(400).json({ error: "item_name is required" });
    }

    const result = await liveOps.submitFoodRequest(eventId, item_name, requested_by);

    res.status(result.action === "created" ? 201 : 200).json({
      message: result.action === "created"
        ? `Request for "${item_name}" sent to kitchen!`
        : `Request for "${item_name}" upvoted! (${result.request.upvotes} votes)`,
      ...result,
    });
  } catch (err) {
...
// ─── Kitchen Food Requests: Explicit Upvote ────────────────────

router.post("/kitchen/:eventId/upvote/:requestId", async (req, res) => {
  try {
    const { requestId } = req.params;
    const updated = await liveOps.upvoteFoodRequest(requestId);
    res.json({ message: `Upvoted! (${updated.upvotes} votes)`, request: updated });
  } catch (err) {
    console.error("Error upvoting food request:", err);
    res.status(400).json({ error: err.message });
  }
});

// ─── Kitchen Food Requests: Leaderboard ────────────────────────

router.get("/kitchen/:eventId/leaderboard", async (req, res) => {
  try {
    const leaderboard = await liveOps.getFoodLeaderboard(req.params.eventId);
    res.json({
      leaderboard,
      total: leaderboard.length,
    });
  } catch (err) {
    console.error("Error fetching food leaderboard:", err);
    res.status(500).json({ error: "Failed to fetch leaderboard" });
  }
});

// ─── Kitchen Food Requests: Update Status ──────────────────────

router.patch("/kitchen/request/:requestId/status", async (req, res) => {
  try {
    const { status } = req.body;
    if (!status) return res.status(400).json({ error: "status is required" });

    const request = await liveOps.updateFoodRequestStatus(req.params.requestId, status);
    res.json({ message: `Request marked as ${status}`, request });
  } catch (err) {
    console.error("Error updating food request status:", err);
    res.status(400).json({ error: err.message });
  }
});
